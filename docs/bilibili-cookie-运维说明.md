...
| 多账号说明 | 若已创建多个 B站账号，先在面板的 `账号槽位` 下拉中选中目标账号再保存或同步 |
...
| 1 | 打开系统调度页 `/scheduler` |
| 2 | 找到 `B站登录态` 面板 |
| 3 | 如果要更新某个指定账号，先在 `账号槽位` 下拉中选中目标账号；未选择额外账号时默认写入 `默认账号` |
| 4 | 点击 `打开浏览器登录并同步` |
| 5 | 系统会打开本机真实浏览器，并复用运行目录下的持久化登录态 |
| 6 | 如果浏览器里还没登录，就在打开的 B站页面里完成登录/验证 |
| 7 | 系统检测到 `SESSDATA` 后，会自动保存浏览器状态，并把最新 Cookie 写入对应账号槽位的 `data/app.env` 键 |
| 8 | 页面返回成功提示 `已从浏览器同步最新B站登录态` 后即可继续采集 |
...
| 6 | 回到系统调度页，先选好目标 `账号槽位`，再把结果粘贴到 `B站登录态` 文本框并保存 |
...
| 9 | 回到系统调度页，先选好目标 `账号槽位`，再粘贴到 `B站登录态` 后保存 |

## 系统内填写要求

| 要求 | 说明 |
| --- | --- |
| 多账号时先选 `账号槽位` | 不同账号会分别写入不同环境变量键 |
| 默认账号 | 写入 `BILIBILI_COOKIE=` |
| 额外账号 | 写入 `BILIBILI_COOKIE__账号KEY=`，例如 `BILIBILI_COOKIE__CREATOR_A=` |
